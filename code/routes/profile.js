...
router.get("/myRecipes/:ids", async (req, res, next) => {
  try {
    ...
    idaArray.map((id) => {
      promises.push(
        profile_utils.getPersonalRecipesFullDetails(req.user, id, "my")
      );
    });
...
/**
 * add new recpie to my recipes
 * @return success or err
 */
router.put("/myRecipes/add", async (req, res, next) => {
  try {
    await profile_utils.addNewRecipe(req.body, req.user, "my");
    res.send("sucsess");
  } catch (err) {
    next(err);
  }
...
/**
 * add new recpie to my family recipes
 * @return success or err
 */
router.put("/familyRecipes/add", async (req, res, next) => {
  try {
    await profile_utils.addNewRecipe(req.body, req.user, "family");
    res.send("sucsess");
  } catch (err) {
    next(err);
  }
